...
const SqlMigrator = require("./SqlMigrator.js");
const SqlClientFactory = require("../../SqlClient/lib/SqlClientFactory");
const fileHelp = require("../../util/file.help");
 ...
 * @extends {SqlMigrator}
 */
class KnexMigrator extends SqlMigrator {
  /**
  ...
  _getWorkingEnvDir(args) {
    return path.join(this.project.folder, 'server', 'tool', args.dbAlias, 'migrations');
  }
  ...
  async _initDbOnFs(args) {
    this.emit(
      "Creating folder: ",
      path.join(this.project.folder, 'server', 'tool', args.dbAlias, 'migrations')
    );
    ...
    try {
      ...
      this.emit(
        "Creating folder: ",
        path.join(this.project.folder, 'server', 'tool', args.dbAlias, this.project.meta.seedsFolder)
      );
      ...
      this.emit(
        "Creating folder: ",
        path.join(this.project.folder, 'server', 'tool', args.dbAlias, this.project.meta.queriesFolder)
      );
      ...
      this.emit(
        "Creating folder: ",
        path.join(this.project.folder, 'server', 'tool', this.project.meta.apisFolder)
      );
  ...
  async _cleanFs(args) {
    this.emit("Removing folder: ", path.join(this.project.folder, 'server', 'tool', args.dbAlias));
    
    try {
      await promisify(rmdir)(path.join(this.project.folder, 'server', 'tool', args.dbAlias));
    } catch (e) {
      log.debug(
        "Error removing folder:",
        path.join(this.project.folder, 'server', 'tool', args.dbAlias),
        e
      );
    }
  }
  ...
  async _initDbWithSql(connectionConfig) {
    ...
    this.emit(
      `Creating Table if not exists in ${connectionConfig.meta.tableName}`
    );
    await sqlClient.createTableIfNotExists({
      tableName: connectionConfig.meta.tableName
    });
  ...
  async _cleanDbWithSql(connectionConfig) {
    ...
    // await sqlClient.createTableIfNotExists({tableName: connectionConfig.meta.tableName});
  }
  ...
  async _initAllEnvDbsWithSql() {
    // const {envs} = this.project;
    
    for (const env in this.project.envs) {
      // this.emit(`Initialising env: ${env}`);
      // for (let i = 0; i < envs[env].length; ++i) {
      //   const connectionConfig = envs[env][i];
      //   await this._initDbWithSql(connectionConfig);
      // }
      await this._initEnvDbsWithSql(env)
    }
  }
  ...
  async _migrationsUp(args) {
    ...
    try {
      ...
      const migrations = await sqlClient.selectAll(connection.meta.tableName);
      ...
      if (files.length === migrations.length) {
        ...
        try {
          ...
          for (
            ...
            const fileNameDown = splitsDown[splitsDown.length - 1];
            ...
            if (onlyList) {
              ...
              const upStatement = await promisify(fs.readFile)(
                files[i],
                "utf8"
              );
          ...
          if (onlyList) {
            ...
            try {
              ...
              for (let data of metaTableInserts) {
                await trx(connection.meta.tableName).insert(data);
  ...
  async _migrationsDown(args) {
    ...
    try {
      // get all evolutions from fs
      const files = await promisify(glob)(args.downFilesPattern);
        ...
        try {
          ...
          for (
            ...
            if (onlyList) {
              ...
              const downStatement = await promisify(fs.readFile)(
                files[i],
                "utf8"
              );
              if (args.sqlContentMigrate)
          ...
          try {
            ...
            for (let condition of metaDownDeletes) {
              ...
              await trx(connection.meta.tableName).where(condition).del();
            }
  ...
  async init(args) {
    ...
    try {
      ...
      await this._initAllEnvOnFilesystem(args);
    } catch (e) {
  ...
  async migrationsUp(args = {}) {
    ...
    return await this._migrationsUp({
      ...
      upFilesPattern: path.join(this.project.folder, 'server', 'tool', args.dbAlias, 'migrations', '*.up.sql'),
      downFilesPattern: path.join(this.project.folder, 'server', 'tool', args.dbAlias, 'migrations', '*.down.sql'),
      tableName: `${this.project.folder}`,
  ...
  async migrationsToSql(args) {
    ...
    try {
      ...
      const upFilePath = path.join(this.project.folder, 'server', 'tool', args.dbAlias, 'migrations', args.title);
      const downFilePath = path.join(this.project.folder, 'server', 'tool', args.dbAlias, 'migrations', args.titleDown);
  ...
  async _writeProjectJson(folder, json) {
  ...
  async migrationsRenameProjectKey(args) {
    ...
    try {
      ...
      if (args.key in this.project) {
        ...
        await this._writeProjectJson(this.project.folder, this.project);
      }
  ...
  async migrationsRenameEnv(args) {
  ...
  async migrationsUpdateEnvDb(args) {
    throw new Error('Not implemented');
  }
  
  async migrationsDeleteEnvDb(args) {
    throw new Error('Not implemented');
  }
  
  async migrationsUpWithExternalClient(args) {
    throw new Error('Not implemented');
  }
  
  async migrationsDownWithExternalClient(args) {
    throw new Error('Not implemented');
  }
