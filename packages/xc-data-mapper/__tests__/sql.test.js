...
describe('xc-data-mapper tests', function () {
  ...
  it('groupBy', async function () {
    let data = await city.groupBy({columnName: 'country_id', limit: 2});
    expect(data[0]['count']).to.not.equal(0);
    expect(data.length).to.be.equal(2);
    data = await city.groupBy({columnName: 'country_id', having: '(count,gt,50)'});
    expect(data.length).to.be.equal(2);
  });

  it('aggregate', async function () {

    let data = await city.aggregate({columnName: 'country_id', fields: 'country_id', func: 'count,sum,avg'});
    expect(data[0]['count']).to.not.equal(0);
  ...
  it('distinct', async function () {
    ...
    let data = await city.distinct({columnName: 'country_id'});
    expect(Object.keys(data[0]).length).to.be.equal(1);
    expect(data[0]['country_id']).to.not.equal(0);

    data = await city.distinct({columnName: 'country_id', fields: 'city'});
  ...
  it('distribution', async function () {
    ...
    let data = await city.distribution({columnName: 'country_id', steps: '0,20,40,100'});
    ...
    data = await city.distribution({columnName: 'country_id', step: 20, min: 0, max: 100});
